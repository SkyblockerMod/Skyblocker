...
class TopBarWidget extends AbstractContainerWidget {
	...
	TopBarWidget(int width, WidgetsConfigurationScreen parent) {
		...
		locationDropdown = new CustomDropdownWidget<>(width / 2 - 100 - 5, 0, 100, 200, List.copyOf(WidgetManager.ALLOWED_LOCATIONS), parent::setCurrentLocation, parent.getCurrentLocation());
	...
	static void drawButtonBorder(GuiGraphicsExtractor context, int x, int y, int y2) {
		context.verticalLine(x - 1, y, y2, ARGB.color(15, -1));
		context.verticalLine(x, y, y2, ARGB.color(100, 0));
		context.verticalLine(x + 1, y, y2, ARGB.color(15, -1));
	}
