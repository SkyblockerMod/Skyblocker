	...
	public void updateContent(List<Component> lines) {
		...
		for (int i = 1; i < lines.size(); i++) {
			...
			switch (string.toLowerCase(Locale.ENGLISH)) {
				...
				case String s when s.contains("gems") -> this.addComponent(Elements.iconTextComponent(Ico.EMERALD, text));
				case String s when s.contains("crystals") -> this.addComponent(Elements.iconTextComponent(Ico.EMERALD, text));
				case String s when s.contains("copper") -> this.addComponent(Elements.iconTextComponent(Ico.COPPER, text));
				case String s when s.contains("garden") -> this.addComponent(Elements.iconTextComponent(Ico.EXPERIENCE_BOTTLE, text));
				case String s when s.contains("fairy") -> this.addComponent(Elements.iconTextComponent(ItemRepository.getItemStack("PLACEABLE_FAIRY_SOUL_RIFT", Ico.FAIRY_SOUL), text));
				case String s when s.contains("rain") -> this.addComponent(Elements.iconTextComponent(Ico.WATER, text));
				case String s when s.contains("brood") -> this.addComponent(Elements.iconTextComponent(Ico.SPIDER_EYE, text));
