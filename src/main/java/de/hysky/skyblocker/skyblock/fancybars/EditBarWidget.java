...
public class EditBarWidget extends AbstractContainerWidget {
		...
		public EditBarWidget(int x, int y, Screen parent) {
				super(x, y, 100, 121, Component.literal("Edit bar"), AbstractScrollArea.defaultSettings(4));
				this.parent = parent;
				...
				// Icon cycles: OFF → LEFT → RIGHT → CUSTOM
				StatusBar.IconPosition[] iconValues = {
						StatusBar.IconPosition.OFF,
						StatusBar.IconPosition.LEFT,
						StatusBar.IconPosition.RIGHT,
						StatusBar.IconPosition.CUSTOM
				};
				iconOption = new EnumCyclingOption<>(0, 11, getWidth(), translatable, iconValues);
				...
				// Text cycles: OFF → BAR LEFT → BAR CENTER → BAR RIGHT → CUSTOM (CENTER excluded)
				StatusBar.TextPosition[] textValues = {
						StatusBar.TextPosition.OFF,
						StatusBar.TextPosition.LEFT,
						StatusBar.TextPosition.BAR_CENTER,
						StatusBar.TextPosition.RIGHT,
						StatusBar.TextPosition.CUSTOM
				};
				textOption = new EnumCyclingOption<>(0, 22, getWidth(), translatable, textValues);
				...
				// Account for both Hide and Show labels width
				contentsWidth = Math.max(contentsWidth, textRenderer.width(Component.translatable("skyblocker.bars.config.hide")) + 9 + 10);
		...
		protected void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
				...
				TooltipRenderUtil.extractTooltipBackground(context, 0, 0, getWidth(), getHeight(), null);
				nameWidget.extractRenderState(context, mouseX, mouseY, delta);
				for (AbstractWidget option : options) option.extractRenderState(context, mouseX - getX(), mouseY - getY(), delta);
				matrices.popMatrix();
		}
		...
		public void setStatusBar(StatusBar statusBar) {
				...
				// Toggle between Hide and Show based on current enabled state
				hideOption.active = true;
				if (statusBar.enabled) {
						hideOption.setMessage(Component.translatable("skyblocker.bars.config.hide"));
						...
						hideOption.setMessage(Component.translatable("skyblocker.bars.config.show"));
						hideOption.setRunnable(() -> {
								statusBar.enabled = true;
								statusBar.anchor = null; // make free-floating
								if (statusBar.width <= 0) statusBar.width = 0.2f;
								FancyStatusBars.updatePositions(true);
						});
				}
		...
		public class RunnableOption extends AbstractWidget {
				...
				protected void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
						...
						Font textRenderer = Minecraft.getInstance().font;
						context.text(textRenderer, getMessage(), getX() + 1, getY() + 1, active ? CommonColors.WHITE : CommonColors.GRAY, true);
				}
		...
		public static class EnumCyclingOption<T extends Enum<T>> extends AbstractWidget {
				...
				/** Constructor using a custom (possibly filtered) set of values to cycle through. */
				public EnumCyclingOption(int x, int y, int width, Component message, T[] values) {
						super(x, y, width, 11, message);
						this.values = values;
						current = values[0];
				}
				...
				protected void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
						...
						Font textRenderer = Minecraft.getInstance().font;
						context.text(textRenderer, getMessage(), getX() + 1, getY() + 1, CommonColors.WHITE, true);
						String string = current.toString();
						context.text(textRenderer, string, getRight() - textRenderer.width(string) - 1, getY() + 1, CommonColors.WHITE, true);
				}
				...
				@Override
				public void onClick(MouseButtonEvent click, boolean doubled) {
						// Cycle only within the filtered values array
						int idx = 0;
						for (int i = 0; i < values.length; i++) {
								if (values[i] == current) { idx = i; break; }
						}
						current = values[(idx + 1) % values.length];
		...
		public static class BooleanOption extends AbstractWidget {
				...
				protected void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
						...
						Font textRenderer = Minecraft.getInstance().font;
						context.text(textRenderer, getMessage(), getX() + 1, getY() + 1, active ? -1 : CommonColors.GRAY, true);
						GuiHelper.border(context, getRight() - 10, getY() + 1, 9, 9, active ? -1 : CommonColors.GRAY);
