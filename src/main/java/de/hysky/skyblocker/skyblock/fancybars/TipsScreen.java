...
public class TipsScreen extends Screen {
		...
		protected void init() {
				...
				prevButton = addRenderableWidget(
								Button.builder(Component.literal("←"), _ -> navigate(-1))
												.bounds(dx + PAD, btnY, 26, 16)
												.build());

				addRenderableWidget(
								Button.builder(Component.literal("Close"), _ -> minecraft.setScreen(parent))
		...
		public void extractRenderState(GuiGraphicsExtractor ctx, int mouseX, int mouseY, float delta) {
				...
				// Title
				ctx.centeredText(font, "Tips & Tricks", dx + DIALOG_W / 2, dy + PAD, COLOR_TITLE);
				...
				ctx.centeredText(font, pageStr, dx + DIALOG_W / 2, dy + 27, COLOR_PAGE);
				...
				for (FormattedCharSequence line : lines) {
						ctx.text(font, line, dx + PAD, textY, COLOR_TEXT, false);
