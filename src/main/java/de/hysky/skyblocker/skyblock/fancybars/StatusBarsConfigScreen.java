...
public class StatusBarsConfigScreen extends Screen {
		...
		public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
				...
				editBarWidget.extractRenderState(context, mouseX, mouseY, delta);
				...
				if (cursorBar != null) {
						...
						context.text(minecraft.font, posText, labelX, labelY, BAR_SEL_COLOR, false);
						...
						context.text(minecraft.font, overlayText, lx, aboveY, BAR_SEL_COLOR, false);
		...
		private void drawBarOutlineWithArrows(GuiGraphicsExtractor ctx, StatusBar bar) {
		 ...
		 */
		private void drawCustomElementHandles(GuiGraphicsExtractor ctx, ScreenRectangle area, boolean isText) {
		...
		private static void fillLeftArrow(GuiGraphicsExtractor ctx, int tipX, int midY, int color) {
		...
		private static void fillRightArrow(GuiGraphicsExtractor ctx, int tipX, int midY, int color) {
				ctx.fill(tipX,     midY,     tipX + 1, midY + 1, color);
				ctx.fill(tipX - 1, midY - 1, tipX,     midY + 2, color);
				ctx.fill(tipX - 4, midY - 2, tipX - 1, midY + 3, color);
		}
		private static void fillUpArrow(GuiGraphicsExtractor ctx, int midX, int tipY, int color) {
		...
		private static void fillDownArrow(GuiGraphicsExtractor ctx, int midX, int tipY, int color) {
				ctx.fill(midX,     tipY,     midX + 1, tipY + 1, color);
		...
		protected void init() {
				...
				this.addRenderableWidget(Button.builder(Component.literal("?"),
								_ -> minecraft.setScreen(new TipsScreen(this)))
