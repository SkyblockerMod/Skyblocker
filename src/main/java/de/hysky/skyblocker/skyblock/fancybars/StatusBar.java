...
public class StatusBar implements Widget, Drawable, Element, Selectable {
	...
	protected void drawBarFill(DrawContext context, int barX, int barWith) {
		HudHeper.renderNineSliceColored(context, BAR_FILL, barX + 1, y + 2, (int) ((barWith - 2) * fill), 5, transparency(colors[0].getRGB()));


		if (hasOverflow() && overflowFill > 0) {
			HudHeper.renderNineSliceColored(context, BAR_FILL, barX + 1, y + 2, (int) ((barWith - 2) * Math.min(overflowFill, 1)), 5, transparency(colors[1].getRGB()));
		}
	}
