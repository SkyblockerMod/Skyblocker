...
public class StatusBar implements LayoutElement, Renderable, GuiEventListener, NarratableEntry {
		...
		public void renderBar(GuiGraphicsExtractor context) {
		...
		public void renderCustomIcon(GuiGraphicsExtractor context) {
		...
		protected void drawBarFill(GuiGraphicsExtractor context, int barX, int barWidth) {
		 ...
		 */
		protected static void fillRounded(GuiGraphicsExtractor ctx, int x, int y, int w, int h, int r, int color) {
				if (w <= 0 || h <= 0) return;
		 ...
		 */
		protected static void fillRoundedClipped(GuiGraphicsExtractor ctx, int x, int y, int w, int h, int r, int fillW, int color) {
		...
		public void renderText(GuiGraphicsExtractor context) {
				...
				if (textPosition == TextPosition.CUSTOM && textCustomScale != 1.0f) {
						...
						GuiHelper.outlinedText(context, Component.translationArg(text), 0, 0, color, outlineColor);
						context.pose().popMatrix();
				} else {
						GuiHelper.outlinedText(context, Component.translationArg(text), x, y, color, outlineColor);
				}
		}

		public void renderCursor(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
				int temp_x = renderX;
				int temp_y = renderY;
		...
		public static class ManaStatusBar extends StatusBar {
				...
				@Override
				protected void drawBarFill(GuiGraphicsExtractor context, int barX, int barWith) {
