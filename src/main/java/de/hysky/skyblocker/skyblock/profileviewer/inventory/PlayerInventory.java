...
public class PlayerInventory implements ProfileViewerPage {
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY, float delta, int rootX, int rootY) {
		...
		tooltip.clear();
		drawContainerItems(graphics, rootX, rootY + 108, IntIntPair.of(1, 4), 36, 40, mouseX, mouseY);
		drawContainerItems(graphics, rootX, rootY + 2, IntIntPair.of(4, 9), 0, 36, mouseX, mouseY);
		drawContainerItems(graphics, rootX + 90, rootY + 108, IntIntPair.of(1, 4), 40, containerList.size(), mouseX, mouseY);
	...
	private void drawContainerTextures(GuiGraphics graphics, String containerName, int rootX, int rootY, IntIntPair dimensions) {
		if (containerName.equals("inventory")) {
			...
			graphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, rootX, rootY, 0, 0, dimensions.rightInt() * 18 + 7, 14, 256, 256);
			graphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, rootX + dimensions.rightInt() * 18 + 7, rootY + dimensions.leftInt() * 18 + 21, 169, 215, 7, 7, 256, 256);
		} else {
			graphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, rootX, rootY, 0, 0, dimensions.rightInt() * 18 + 7, dimensions.leftInt() * 18 + 17, 256, 256);
			graphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, rootX + dimensions.rightInt() * 18 + 7, rootY, 169, 0, 7, dimensions.leftInt() * 18 + 17, 256, 256);
