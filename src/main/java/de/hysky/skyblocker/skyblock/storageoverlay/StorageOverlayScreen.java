...
public class StorageOverlayScreen extends AbstractContainerScreen<StorageOverlayScreenHandler> {
	...
	public static void setup() { //already had init therefore called setup
		...
		ClientSendMessageEvents.MODIFY_COMMAND.register(command -> {
			if (!SkyblockerConfigManager.get().uiAndVisuals.storageOverlay.enabled || savedIndex < 0 || Minecraft.getInstance().screen instanceof StorageOverlayScreen || !command.equals("storage")) {
				return command;
			} else {
