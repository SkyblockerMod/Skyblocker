...
public class ConfigFix2QuickNav extends ConfigDataFix {
	...
	private <T> Dynamic<T> fixButton(Dynamic<T> button) {
		return button.renameAndFixField("item", "itemData", itemData -> itemData.renameAndFixField("id", "item", id -> id.createString(ResourceLocation.parse(id.asString().getOrThrow()).toString())));
	}
}
