...
public class ItemSelectionScreen extends AbstractPopupScreen {
	...
	private final List<Pair<String, String>> skullIcons = List.of(
			...
			Pair.of("The park howling cave", "1832d53997b451635c9cf9004b0f22bb3d99ab5a093942b5b5f6bb4e4de47065"),
			...
			Pair.of("HOTM Heart Of The Mountain", "86f06eaa3004aeed09b3d5b45d976de584e691c0e9cade133635de93d23b9edb"),
			...
			Pair.of("Forgotton Skull", "6becc645f129c8bc2faa4d8145481fab11ad2ee75749d628dcd999aa94e7"),
			...
			Pair.of("Dwarven Mines Base Camp", "2461ec3bd654f62ca9a393a32629e21b4e497c877d3f3380bcf2db0e20fc0244")
	);
	...
	protected void init() {
		...
		doneButton = ButtonWidget.builder(ScreenTexts.DONE, b -> {
			...
			target.components = "[" + String.join(",", selectedItem.item.getComponentChanges().entrySet().stream()
					.filter(entry -> entry.getValue().isPresent())
					.map(entry -> {
						ComponentType<Object> key = (ComponentType<Object>) entry.getKey();
						Object o = entry.getValue().get();
						return Registries.DATA_COMPONENT_TYPE.getId(entry.getKey()) + "=" + key.getCodec().encodeStart(NbtOps.INSTANCE, o).getOrThrow().toString();
					}).toList()) + "]";
			close();
		}).build();
