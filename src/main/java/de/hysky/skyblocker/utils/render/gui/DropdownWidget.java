...
public class DropdownWidget<T> extends AbstractContainerWidget {
	...
	public DropdownWidget(Minecraft minecraftClient, int x, int y, int width, int maxHeight, int entryHeight, List<T> entries, Consumer<T> selectCallback, T selected, Consumer<Boolean> openedCallback) {
		super(x, y, width, 0, Component.empty(), defaultSettings(4));
	...
	protected void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
		...
		extractHeader(graphics, mouseX, mouseY, delta);
		if (isMouseOver(mouseX, mouseY)) graphics.requestCursor(CursorTypes.POINTING_HAND);
	}

	protected void extractHeader(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
		graphics.fill(getX(), getY(), getRight(), getY() + headerHeight + 1, CommonColors.BLACK);
		GuiHelper.border(graphics, getX(), getY(), getWidth(), headerHeight + 1, CommonColors.WHITE);
