...
public class TitleContainerConfigScreen extends HudConfigScreen {
	...
	protected void renderWidget(GuiGraphicsExtractor graphics, List<AbstractWidget> widgets, float delta) {
		...
		graphics.centeredText(font, "Right Click To Reset Position", width / 2, font.lineHeight * 5 + 15, Color.GRAY.getRGB());
		...
		graphics.horizontalLine(x1, x2, y1, Color.RED.getRGB());
		graphics.horizontalLine(x1, x2, y2, Color.RED.getRGB());
		graphics.verticalLine(x1, y1, y2, Color.RED.getRGB());
		graphics.verticalLine(x2, y1, y2, Color.RED.getRGB());
	}
