...
public class ApiAuthentication {
	...
	private static void updateToken() {
		ProfileKeyPairManager profileKeys = ((MinecraftClientAccessor) CLIENT).getProfileKeyPairManager();
		...
		profileKeys.prepareKeyPair().thenAcceptAsync(playerKeypairOpt -> {
			...
			if (playerKeypairOpt.isPresent() && !expired) {
				...
				try {
					...
					logErrorAndScheduleRetry(Component.translatable("skyblocker.api.token.authFailure"), 900 * 20, "[Skyblocker Api Auth] Failed to refresh the api token! Some features might not work :(", e);
				}
			} else {
