...
public class ApiAuthentication {
		...
		CLIENT.getProfileKeys().fetchKeyPair().thenAcceptAsync(playerKeypairOpt -> {
			if (playerKeypairOpt.isPresent()) {
				...
				try {
					...
					RenderHelper.runOnRenderThread(() -> TOKEN_REQUEST_CALLBACK.invoker().callback());
	...
	public static final Event<TokenRequestCallback> TOKEN_REQUEST_CALLBACK = EventFactory.createArrayBacked(TokenRequestCallback.class, listeners -> () -> {
		for (TokenRequestCallback listener : listeners) {
			listener.callback();
		}
	});
