...
public abstract class MutableComponentMixin {
	...
	public FormattedCharSequence getVisualOrderText(Language instance, FormattedText formattedText, Operation<FormattedCharSequence> original) {
		if (getContents() instanceof GridComponent.Contents(String group, List<Component> components))
			return new GridFormattedCharSequence(group, components.stream().map(Component::getVisualOrderText).toList());
