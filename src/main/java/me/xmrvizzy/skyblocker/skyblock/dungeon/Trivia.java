...
public class Trivia {
    ...
    private static final Pattern TRIVIA_QUESTION = Pattern.compile("^ +(?:([A-Za-z' ]*\\?)|§6 ([ⓐⓑⓒ]) §a([a-zA-Z0-9 ]+))$");
    private static final Map<String, String[]> answers;
    private List<String> solutions = null;

    public void process(String message, CallbackInfo ci) {
        assert client.player != null;
        Matcher matcher = TRIVIA_QUESTION.matcher(message);
        if (!matcher.matches())
            return;
        ...
        if (answer != null) {
            if (!solutions.contains(answer)) {
                client.player.sendMessage(new LiteralText("     " + Formatting.GOLD + matcher.group(2) + Formatting.RED + " " + answer), false);
                ci.cancel();
            }
        } else {
            String question = matcher.group(1);
            if (question != null)
                updateSolutions(matcher.group(1));
        }
    }
    ...
    static {
        ...
        answers.put("How many total Fairy Souls are there?", new String[]{"220 Fairy Souls"});
        ...
        answers.put("How many Fairy Souls are there in Mushroom Desert?", new String[]{"8 Fairy Souls"});
