...
class StatusBarTrackerTest {
    ...
    void normalStatusBar() {
        String res = StatusBarTracker.updateNormal("§c934/1086❤     §a159§a❈ Defense     §b562/516✎ Mana", false);
        assertNull(res);
    ...
    @Test
    void overflowMana() {
        String res = StatusBarTracker.updateNormal("§61605/1305❤     §a270§a❈ Defense     §b548/548✎ §3200ʬ", false);
        assertNull(res);
    ...
    @Test
    void regeneration() {
        String res = StatusBarTracker.updateNormal("§c2484/2484❤+§c120▄     §a642§a❈ Defense     §b2557/2611✎ Mana", false);
    ...
    void instantTransmission() {
        ...
        assertEquals("§b-20 Mana (§6Instant Transmission§b)", StatusBarTracker.updateNormal(actionBar, false));
        assertNull(StatusBarTracker.updateNormal(actionBar, true));
    }

    @Test
    void rapidFire() {
        String actionBar = "§c2509/2509❤     §b-48 Mana (§6Rapid-fire§b)     §b2739/2811✎ Mana";
        assertEquals("§b-48 Mana (§6Rapid-fire§b)", StatusBarTracker.updateNormal(actionBar, false));
        assertNull(StatusBarTracker.updateNormal(actionBar, true));
    }
    ...
    void zombieSword() {
        ...
        assertEquals("§b-56 Mana (§6Instant Heal§b)     §e§lⓩⓩⓩⓩ§6§lⓄ", StatusBarTracker.updateNormal(actionBar, false));
        assertEquals("§e§lⓩⓩⓩⓩ§6§lⓄ", StatusBarTracker.updateNormal(actionBar, true));
    }

    @Test
    void campfire() {
        String res = StatusBarTracker.updateNormal("§c17070/25565❤+§c170▃   §65,625 DPS   §c1 second     §b590/626✎ §3106ʬ", false);
